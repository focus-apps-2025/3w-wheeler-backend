  ...
  export const getResponsesByForm = async (req, res) => {
    try {
      ...
      // If user is owner or superadmin, we use the standard tenantFilter
      // If user is shared via chassis, we bypass tenantFilter (since responses belong to owner) 
      // and then manually filter by chassis below.
      if (isOwner || isSuperAdmin) {
        Object.assign(query, req.tenantFilter);
      } else if (!isShared && !hasChassisShare) {
        // If none of the above, access should have been caught by formSearchQuery, 
        // but just in case, ensure we only see our own stuff.
        Object.assign(query, req.tenantFilter);
      }
